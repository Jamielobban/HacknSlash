...
public class PlayerControl : MonoBehaviour
{
    ...
    void CheckPerfectHit()
    {
        ...
        if ((Time.time - attackStartTime) >= currentComboAttacks.attacks[currentComboAttack].delay && !flash.isPlaying && !atackPress && damageMult == 1 && (Time.time - attackStartTime) <= currentComboAttacks.attacks[currentComboAttack].delay + delayDa�o)
        {
            flash.startLifetime = delayDa�o;
            for (int i = 0; i < flash.gameObject.transform.childCount; i++)
            {
                flash.gameObject.transform.GetChild(i).GetComponent<ParticleSystem>().startLifetime = delayDa�o;
            }


            flash.Play();
        }
        if ((Time.time - attackStartTime) >= currentComboAttacks.attacks[currentComboAttack].delay + delayDa�o && flash.isPlaying)
        {

        }

        if ((Time.time - attackStartTime) >= currentComboAttacks.attacks[currentComboAttack].delay && damageMult == 1 && (Time.time - attackStartTime) <= currentComboAttacks.attacks[currentComboAttack].delay + delayDa�o)
        {
        ...
        if ((Time.time - attackStartTime) >= currentComboAttacks.attacks[currentComboAttack].delay + delayDa�o && atackPress)
        {
            atackPress = false;
        ...
        if (controller.Teleport1)
        {
            ...
            if (pos2 != Vector3.zero)
            {
                ...
                if (Physics.Raycast(pos, transform.TransformDirection(-this.transform.up), out hit, 2000, 1 << 7))
                {
                    ...
                    if ((Time.time - attackStartTime) >= delay + delayDa�o)
                    {
                        damageMult = 1;
